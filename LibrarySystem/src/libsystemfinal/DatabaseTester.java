...
public class DatabaseTester {
	...
	public static void main(String[] args){
		...
		System.out.println("getAccount -- non-existent account");
		Account a3 = dbc.getAccount("doesnotexist");
		if(a3 == null) {
			System.out.println("Correctly returned null.");
		}
		else {
			System.out.println("Incorrectly returned account object.");
		}
		System.out.println();
		
		//Test getAccount for empty string account name
		System.out.println("getAccount -- empty string for name");
		Account a4 = dbc.getAccount("");
		if(a4 == null) {
			System.out.println("Correctly returned null.");
		}
		else {
		...
		Shelf s1 = new Shelf(1, "Fantasy", 100, new ArrayList<Book>());
		if(dbc.putShelf(s1)) {
		...
		Shelf s2 = dbc.getShelf(2);
		if(s2 != null) {
			System.out.println("Genre: " + s2.getGenre());
			System.out.println("Capacity: " + s2.getCapacity());
			List<Book> books = s2.getBooks();
			if(books != null) {
				for(int i = 0; i < books.size(); i++) {
					System.out.println("\tTitle: " + books.get(i).getTitle() + " Author: " + books.get(i).getTitle());
				}
			}
		}
		else {
		...
		//Test getShelf non-existent shelf
		System.out.println("getShelf -- non-existent shelf");
		Shelf s3 = dbc.getShelf(-1);
		if(s3 != null) {
			System.out.println("Shelf object returned.");
		}
		else {
			System.out.println("Correctly returned null.");
		}
